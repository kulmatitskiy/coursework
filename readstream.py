...
def getShow(line):
  ...
  for word in words:
    try:
      show = shows[word.replace("#","").replace("@","")].encode('ascii')
      return(show)
    except:
      None
...
def postToSolr(t, rdd):
  solr_url = "http://auisbigdatabox.cloudapp.net:8983/solr/project/update"
...
ods = ds.window(30, 10).map(lambda line: getText(line)) \
